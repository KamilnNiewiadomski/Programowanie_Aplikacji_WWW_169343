...
{
        /* zapytanie */
	$query = "DELETE FROM `category` WHERE `category`.`id` = '$id' OR `category`.matka = '$id' LIMIT 1";
        /* wynik */
        $results = mysqli_query($link,$query);
}
/* Edytuje kategorie */
...
/* Pokazuje kategorie bez id i matki*/
function PokazKategorie_sklep($link, $filtr, $odstep)
{
        /* zapytanie */
        $query="SELECT * FROM category WHERE `matka` = $filtr";
        /* wynik */
        ...
        while($row = mysqli_fetch_array($result))
	{
		...
		if($filtr == $row['id'])
			$odstep += 100;
		PokazKategorie_sklep($link, $row['id'], $odstep);
        }
}
